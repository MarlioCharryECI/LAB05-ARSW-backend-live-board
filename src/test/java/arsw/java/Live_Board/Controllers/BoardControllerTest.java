...
class BoardControllerTest {
    ...
    @Mock
    private BoardCommunicationFactory communicationFactory;

    @Mock
    private BoardCommunicationStrategy strategy;
    ...
    void clear_ShouldReturnNoContent() throws Exception {
        ...
        mockMvc.perform(post("/api/clear"))
    ...
    void syncBoard_WithoutSince_ShouldReturnChanges() throws Exception {
        Map<String, Object> expectedChanges = Map.of(
            "strokes", List.of(),
            "boardVersion", 1L
        );
        ...
        mockMvc.perform(get("/api/board/sync"))
                .andExpect(status().isOk())
    ...
    void syncBoard_WithSince_ShouldReturnChanges() throws Exception {
        ...
        Map<String, Object> expectedChanges = Map.of(
            "strokes", List.of(createValidStroke()),
            "boardVersion", 2L
        );
    ...
    void heartbeat_ExistingUser_ShouldReturnUserInfo() throws Exception {
        ...
        mockMvc.perform(post("/api/users/{userId}/heartbeat", userId))
                .andExpect(status().isOk())
    ...
    void getBoardInfo_ShouldReturnInfo() throws Exception {
        ...
        mockMvc.perform(get("/api/board/info"))
                .andExpect(status().isOk())
