...
/**
 * Factory que selecciona la estrategia de comunicación del tablero
...
public class BoardCommunicationFactory {
    ...
    @Autowired
    private Map<String, BoardCommunicationStrategy> strategies;

    /**
     * Obtiene la estrategia de comunicación configurada.
     * Por defecto usa REST si no se especifica nada.
     *
    ...
    public BoardCommunicationStrategy getStrategy() {
        ...
        if (strategy == null) {
            throw new IllegalArgumentException("Unknown communication mode: " + mode + 
                ". Available modes: " + strategies.keySet());
        }
